...
const SignUp =() =>{
    ...
    return(
        <form className="flex flex-col mx-4 mb-3" onSubmit={handleSubmit(onSubmit)}>
            <>
                {errors.email || errors.password || errors.fullName && <span className="w-full flex justify-center mb-2.5 text-red-500">Обе поля должны заполнены</span>}
                ...
                <input className="text-black bg-white rounded-2xl px-4 py-2 font-bold text-lg mb-2.5" type="text" {...register("fullName", { required: true }) } required={true}/>
